...
export function Solicitacoes() {
  ...
  async function agir(item:Item,acao:"aprovar"|"recusar"|"converter") { try {
    ...
    if(acao==="converter"&&r.data.assinanteId) navigate(`/assinantes/${r.data.assinanteId}`); else await carregar();
  ...
  return <><h1>Solicitações</h1><div className="filtros"><Select value={produto} onChange={e=>setProduto(e.target.value)} aria-label="Produto">
    ...
    <Mensagem texto={erro}/><div className="lista">{itens.map(i=><Card key={i.id}><div className="linha-titulo"><strong>{i.nomeOrganizacao}</strong><span>{rotularStatus(i.status)}</span></div>
      ...
      {i.status==="APROVADA"&&<Button onClick={()=>agir(i,"converter")}>Converter em assinante</Button>}</Card>)}</div></>;
}
