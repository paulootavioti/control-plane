...
interface ValidacaoMercadoPago { valido: boolean; siteId: string; verificadoEm: string }
interface EvidenciaWebhook { homologado: boolean; assinaturaValidada: boolean; ultimoEvento: null | { tipo: string; acao: string; status: string; recebidoEm: string; processadoEm: string | null } }
...
export function Billing() {
  ...
  const [evidenciaWebhook, setEvidenciaWebhook] = useState<EvidenciaWebhook | null>(null);
  ...
  const carregar = useCallback(async () => {
    try {
      const [resumoResposta, falhasResposta, execucoesResposta, prontidaoResposta, evidenciaResposta] = await Promise.all([
        ...
        api.get<EvidenciaWebhook>("/billing/operacao/mercado-pago/webhook"),
      ]);
      ...
      setEvidenciaWebhook(evidenciaResposta.data);
      setErro("");
    } catch (erroDaBusca) {
  ...
  async function validarMercadoPago() {
    setProcessando("mercado-pago"); setErro(""); setSucesso("");
    try {
      const resposta = await api.post<ValidacaoMercadoPago>("/billing/operacao/mercado-pago/validar");
      setSucesso(`Credencial do Mercado Pago validada para ${resposta.data.siteId} em ${dataHora(resposta.data.verificadoEm)}.`);
    } catch (erroDaAcao) {
      setErro(getApiErrorMessage(erroDaAcao, "Não foi possível validar a credencial do Mercado Pago."));
    } finally { setProcessando(null); }
  }

  if (!resumo || !falhas || !prontidao || !evidenciaWebhook) return <><h1>Operação do billing</h1><Mensagem texto={erro || "Carregando…"} tipo={erro ? "erro" : "vazio"} /></>;
  ...
  return <>
    ...
    <div className="grade-larga secao-operacional">
      <section className="cartao cartao-largo"><h2>Prontidão</h2><p><span className={prontidao.prontoParaHomologacao ? "estado-ok" : "estado-pendente"}>{prontidao.prontoParaHomologacao ? "Pronto para homologação" : "Homologação bloqueada"}</span> · <span className={prontidao.prontoParaProducao ? "estado-ok" : "estado-pendente"}>{prontidao.prontoParaProducao ? "Produção habilitada" : "Produção ainda desabilitada"}</span></p><div className="grade-checklist">{[["Mercado Pago", prontidao.mercadoPago], ["Worker", prontidao.worker], ["Notificações", prontidao.notificacoes]].map(([titulo, itens]) => <div key={titulo as string}><strong>{titulo as string}</strong><ul className="checklist">{Object.entries(itens as Record<string, boolean>).map(([nome, ok]) => <li key={nome} className={ok ? "estado-ok" : "estado-pendente"}>{ok ? "Pronto" : "Pendente"} · {nome}</li>)}</ul></div>)}</div><button className="botao-acao" disabled={!prontidao.mercadoPago.pronto || processando === "mercado-pago"} onClick={() => void validarMercadoPago()}>{processando === "mercado-pago" ? "Validando…" : "Validar credencial Mercado Pago"}</button></section>
      <section className="cartao cartao-largo"><h2>Webhook Mercado Pago</h2>{!evidenciaWebhook.ultimoEvento ? <p className="vazio">Nenhum webhook assinado recebido. Use o simulador do Mercado Pago após configurar a URL de teste.</p> : <><p><span className="estado-ok">Assinatura validada</span></p><p><strong>{evidenciaWebhook.ultimoEvento.tipo}</strong> · {evidenciaWebhook.ultimoEvento.acao}</p><small>Recebido em {dataHora(evidenciaWebhook.ultimoEvento.recebidoEm)} · estado {evidenciaWebhook.ultimoEvento.status}</small></>}</section>
