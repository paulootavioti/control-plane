...
diretorioRoutes.get("/:slug/faturas", autenticarDiretorio, async (request, response) => {
  ...
  try { return response.json(await new ListarFaturasTenantService(prisma).execute(produto.data, slug.data)); }
  catch (erro) {
    if (erro instanceof Error && erro.message === "TENANT_NAO_ENCONTRADO") return response.status(404).json({ codigo: "TENANT_NAO_ENCONTRADO", mensagem: "Tenant não encontrado." });
    throw erro;
  }
});
