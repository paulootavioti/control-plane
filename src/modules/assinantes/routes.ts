...
const novoAssinanteSchema = z.object({
  produtoId: z.enum(["sysbelt", "mecanix", "psyche"]),
...
assinantesRoutes.post(
  ...
  async (request, response) => {
    ...
    try {
      ...
      if (erro instanceof Error && erro.message === "ASSINANTE_NAO_ENCONTRADO") return response.status(404).json({ mensagem: "Assinante não encontrado." });
      if (erro instanceof Error && ["ASSINANTE_NAO_ELEGIVEL", "ASSINATURA_CORRENTE_EXISTE", "PLANO_VERSAO_NAO_ELEGIVEL", "PLANO_PRODUTO_DIVERGENTE", "PERIODO_TESTE_INVALIDO"].includes(erro.message)) {
